...
public class TlvMpuExtractor {
    ...
    protected List<MMTP_Packet> m_fragmented01_mmtp_video_mfu = new ArrayList<>();
    protected List<MMTP_Packet> m_fragmented02_mmtp_video_mfu = new ArrayList<>();
...
//        m_mfu_timed_text_event_thread.start();
     ...
     */
    protected void processMfuVideo(MMTP_Packet mmtp) throws InterruptedException, IOException {
        List<MFU> mfus = null;
        ...
        Logger.d(String.format("[V] pid : 0x%04x, psn : 0x%08x, " + 
                "msn : 0x%08x, f_i : 0x%x, timed_flag : 0x%x, a_f : 0x%x, mfu.size : %d \n", 
                packet_id, mmtp.getPacketSequenceNumber(),
                mpu.getMPUSequenceNumber(), mpu.getFragmentationIndicator(),
...
//                    BinaryLogger.debug(mfus.get(i).MFU_data_byte);
            ...
            case 0x01:
                ...
                break;
            case 0x02:
                m_fragmented02_mmtp_video_mfu.add(mmtp);
                break;
            case 0x03:
                for ( Iterator<MMTP_Packet> it = m_fragmented01_mmtp_video_mfu.iterator() ; 
                        it.hasNext() ; ) {
                    MMTP_Packet mmtp01 = it.next();
                    MMTP_Payload_MPU mpu01 = mmtp01.getMPU();
                    if( mmtp01.getPacketId() == packet_id && 
                ...
                for ( Iterator<MMTP_Packet> it = m_fragmented02_mmtp_video_mfu.iterator() ; 
                        it.hasNext() ; ) {
                    MMTP_Packet mmtp02 = it.next();
                    MMTP_Payload_MPU mpu02 = mmtp02.getMPU();
                    if( mmtp02.getPacketId() == packet_id && 
                            mpu02.getFragmentationIndicator() == 0x02 ) {
...
//                outputStreamNal.write(ByteBuffer.allocate(4).putInt(outputStreamRawMfu.toByteArray().length).array());
    ...
    protected void processMfuAudio(MMTP_Packet mmtp) throws InterruptedException, IOException {
        /**
         * Audio Filtering
         */
    }
    
    protected void processMfuData(MMTP_Packet mmtp) {
        MMTP_Payload_MPU mpu = mmtp.getMPU();
        /**
         * Data Filtering
         */
        Logger.d(String.format("[D] pid : 0x%04x, ptype : 0x%x, psn : 0x%08x, " + 
                ...
                mmtp.getPacketId(), mmtp.getPayloadType(), mmtp.getPacketSequenceNumber(),
